...
code_int free_ent = 0;
int exit_stat = 0;
code_int compressgetcode();
void Usage()
{
  puts("Usage: compress [-dfvcV] [-b maxbits] \n");
}
     ...
     char **argv;
{
  ...
  if (nomagic == 0)
	...
	{
	  puts("Data not in compressed format\n");
	  return (1);
	}
	...
	{
	  puts(
	   "stdin: compressed with too many bits\n");
	  return (1);
	}
    }
  decompress();
...
decompress()
{
  ...
  free_ent = ((block_compress) ? 257 : 256);
  finchar = oldcode = compressgetcode();
  if (oldcode == -1)
    {
      return 0;
    }
